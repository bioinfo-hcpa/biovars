...
abraom<- params$abraom
frequency<-params$frequency
colorfreqheat<-params$colorfreqheat
coloranotheat<-params$colorfreqheat
...
if(abraom==FALSE){
   ...
   data<-filename
   new_names<-c("Variant.ID","rsID", "Gene","Annotation","Chromosome", "Location","Reference","Alternative" ,                     "African", "Amish", "Latino","Ashkenazi.Jewish","East.Asian","European..Finnish.",                               "European..non.Finnish.", "Other","South.Asian","Middle.Eastern","Brazilian.ABraOM") 
...
get_pop_dfs <- function(df=NULL, freq_threshold=frequency){
...
biovars_plot_list<- function(data,map=FALSE,get_info){
  #Data
  `%ni%` = Negate(`%in%`)
  #BIOVARS
  if("Brazilian.ABraOM" %in% colnames(data)){
    df<-data
  }
  #pynoma 
   if("Brazilian.ABraOM" %ni% colnames(data)){
     df <- get_reoordered_df(data)
   }

  #Put the data based with or without image.
  if(get_info == FALSE){
  if(map==FALSE){
  ...
  if(get_info==TRUE){
    ...
    pops_vars <- get_pop_var_lists(pop_dfs)
    num_unique_vars <- get_number_of_vars(pops_vars)
    plot_list <- get_all_plots(pops_vars, pop_colors,map,get_info = TRUE)
    return(plot_list)
  }
}

biovars_plot_list(filename,map=TRUE,get_info=FALSE)
pop_info=biovars_plot_list(filename,map=TRUE,get_info=TRUE)
...
```
<img  align="middle" src="rplot_world_view.png" width="1200px" height="600px"  alt="centered image"/>
...
biovars_plot<- function(data,start,end,abraom=FALSE){
...
info<-biovars_plot(data,start,end,abraom=abraom)
# set axis 
# set axis 
y.axisSettings <- list(
  title ="",
...
biovars_plot<- function(data,start,end,mut=F,abraom=FALSE){
  ...
  if(info_length > 80){
    ...
    colours=c('No Variant'='grey95',
      ...
      return(h)}}
info<-biovars_plot(filename,start,end,abraom=abraom)
