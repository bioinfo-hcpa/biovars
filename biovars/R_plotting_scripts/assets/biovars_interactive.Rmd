...
```{r, echo=FALSE}
pdebug <- function(printable){
  cat(printable,file="/home/colombelli/debug.txt",append=TRUE)
}
...
get_pop_dfs <- function(df=NULL, freq_threshold=0.01){
  ...
  for (pop_idx in population_indexes){
    if (is.na(names(df)[pop_idx])) break
...
get_all_plots <- function(pops_variants, pop_colors, get_info){
...
biovars_plot_list<- function(df, get_info){
  ...
  if(get_info) {
    ...
    plot_list <- get_all_plots(pops_vars, pop_colors, get_info)
    return(plot_list)
  }
...
biovars_plot_list(filename, get_info=FALSE)
pop_info=biovars_plot_list(filename, get_info=TRUE)
